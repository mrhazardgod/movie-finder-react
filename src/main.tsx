...
document.addEventListener("click", (e) => {
  ...
  const styles = window.getComputedStyle(editEl);
  const styleSnapshot: Record<string, string> = {};
  ["color", "fontSize", "fontWeight", "backgroundColor", "padding", "margin", "borderRadius"].forEach(
    (prop) => { styleSnapshot[prop] = styles.getPropertyValue(prop); }
  );
  window.parent.postMessage(
    { type: "ELEMENT_SELECTED", editId, text: text.trim(), styles: styleSnapshot },
    "*"
  );
});
